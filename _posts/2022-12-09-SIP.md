...
![](/images/posts/2022-12-09-SIP/oclp-config.png)
...
Where this becomes an issue is developers assuming SIP is only valid with `0xFF` to validate if it has the bits they require. But if a user updated macOS and got a new SIP bit, suddenly the software's hard coded assumption is now incorrect.
