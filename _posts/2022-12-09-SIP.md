...
The raw SIP value is a `UINT32` integer, which is treated as a bitmask by the Operating System.

* But what is a bitmask?
  * The purpose of a bitmask is to easily hold multiple setting configurations inside of 1 value. So if a certain number is found inside the bitmask, we know this setting is "on".
