...
Now with the iMac20,x series, this is the first iMac to be released with offical support for Display Stream Compression (DSC) which would be required to run a 5k panel with 10bit color over a single cable. This is actually the exact same reason Apple couldn't update iMac18,x or iMac19,x with a new single stream panel, as Displayport 1.4 by default does not support enough bandwidth to reach 10bit with 5k.
