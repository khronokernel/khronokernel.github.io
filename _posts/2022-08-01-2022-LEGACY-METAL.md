...
Thankfully with Monterey, no compilers were dropped so only the GPU specific graphics stack needed some work.
...
We'll be testing all 3 combos and commenting on on our results.
...
When downgrading binaries to older versions, you need to keep in mind that other frameworks will be expecting specific functions and such to be available. To help avoid this issue, you can ["shim"](https://en.wikipedia.org/wiki/Shim_(computing)) a binary against the newer version to resolve some of these issues.
